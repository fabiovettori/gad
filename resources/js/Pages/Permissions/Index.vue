<template>
    <app-layout class="relative" :scrollable="true" :title="'Permissions'" :contentStyle="flashMessage ? 'height:calc(100vh - 70px - 56px - 68px);' : 'height:calc(100vh - 70px - 56px);'" @flashActive="flashMessage = true" @flashClose="flashMessage = false">

        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Permissions
                <span class="text-sm font-light">[records: {{ users.length }}]</span>
            </h2>
        </template>
        ...
        <template #controls-bar>
            <div class="flex justify-between controls relative">
                ...
                <filter-records :tableHeader="['name', 'last name', 'email']" @userSearch="search" @reset="searchQuery = ''" @tableHeaderActive="tableHeaderActive" :disabled="filterStatus"/>
            ...
            <table class="table-fixed registries">
                <thead class="bg-gray-500">
                    <tr class="h-14 py-3 rounded">
                        ...
                        <th class="text-left w-40 text-white"> Profile </th>
                ...
                <tbody>
                    <transition-group
                        name="records">
                        <tr v-for="(user, index) in users" :key="user.id" v-show="filter(user)" class="h-14 py-5 border border-gray-300 bg-white hover:bg-gray-100">

                            <td class="relative flex items-center justify-evenly h-14 w-20">
                                <button @click="deleteMe(user.id)" class="uppercase focus:outline-none" type="button">
                                    <span class="relative far fa-times-circle hover:text-purple-500"></span>
                                </button>

                                <inertia-link :href="route('user.edit', {user: user.id})" class="focus:outline-none">
                                    <span class="relative fas fa-pen hover:text-purple-500"></span>
                                </inertia-link>
                            </td>
                            <td class="text-center w-12 px-2"> {{ index + 1 }} </td>
                            <td>
                                <img class="w-12 rounded-lg" :src="`https://eu.ui-avatars.com/api/?name=${user.name}+${user.last_name}&background=8b61f7&color=ffffff&format=svg`" alt="avatar">
                            </td>
                            <td class="w-60 break-words px-2"> {{ user.name }} </td>
                            <td class="w-60 break-words px-2"> {{ user.last_name }} </td>
                            <td class="w-60 break-words px-2"> {{ user.email }} </td>
                            <td class="w-64 break-words px-2 py-3 flex justify-between items-center">
                                <div class="flex justify-center items-center bg-gray-800 rounded-lg py-2 px-3 w-18" v-for="permission in permissions">
                                    <input class="mr-1" type="checkbox" :name="user.name" :value="permission.id" :checked="user.permissions[0].id == permission.id ? true : false" disabled>
                                    <label :for="user.name" class="text-xs font-bold uppercase text-white"> {{ permission.name }} </label>
                                </div>
                            </td>
                        </tr>
                    </transition-group>
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                tableHeader: ['name', 'last name', 'email', 'permissions'],
                closeModal: {
                    status: false,
                    id: null,
                },
        ...
        methods: {
            ...
            permissionsCheck(userPermissions, permissionToFind){
            ...
            filter(user){
                if (this.queryParam === '') {
                    return true;
                } else if (user[this.queryParam].toString().toLowerCase().includes(this.searchQuery.toLowerCase())) {
                    return true;
                }
            }
        }
